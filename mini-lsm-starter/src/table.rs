...
impl SsTable {
    ...
    pub fn open(id: usize, block_cache: Option<Arc<BlockCache>>, file: FileObject) -> Result<Self> {
        let bloom_end = file.1 - 8;

        let max_commit_ts = file.read(bloom_end, 8)?.as_slice().get_u64();

        let bloom_offset = file.read(bloom_end - 4, 4)?.as_slice().get_u32() as u64;
        let bloom_bytes = file.read(bloom_offset, bloom_end - 4 - bloom_offset)?;
    ...
    pub fn read_block(&self, block_idx: usize) -> Result<Arc<Block>> {
        ...
        let next_offset = if block_idx < self.block_meta.len() - 1 {
    ...
    pub fn find_block_idx(&self, key: KeySlice) -> usize {
        for idx in 0..self.block_meta.len() {
            let meta = &self.block_meta[idx];
            if meta.first_key.as_key_slice() <= key && meta.last_key.as_key_slice() >= key {
                return idx;
            }
        }
        panic!("Block index not found")
    }
